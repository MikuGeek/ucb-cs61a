...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        pos = 0
        while True:
            if pos < self.min_range or pos > self.max_range:
                if p.entrance == beehive:
                    return None
                ...
                elif p.entrance is not None and p.entrance != beehive:
                    p = p.entrance
                    pos += 1
                else:
                    return None
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        if p is not None:
            ...
            if self.armor - amount == 0 and len(bees) != 0:
                for i in bees_copy:
...
class HungryAnt(Ant):
    ...
    def eat_bee(self, bee):
        ...
        print("DEBUG: Attack", bee, bee.armor)
        # END Problem 6

    def action(self, gamestate):
        # BEGIN Problem 6
        "*** YOUR CODE HERE ***"
        print("DEBUG: Digest", self.digesting)
...
# The WallAnt class
class WallAnt(Ant):
    """ WallAnt is useful because it has a large armor value."""
...
# END Problem 13
    ...
    def action(self, gamestate):
        ...
        if self.place != None and self.place.exit != None:
            p = self.place.exit
        else:
            p = None
        while p is not None:
            if hasattr(p.ant, "damage") and not hasattr(p.ant, "strong"):
                p.ant.damage *= 2
                p.ant.strong = True
            if isinstance(p.ant, ContainerAnt) and p.ant.contained_ant != None and not hasattr(p.ant.contained_ant, "strong"):
                p.ant.contained_ant.damage *= 2
                p.ant.contained_ant.strong = True
            p = p.exit
        # END Problem 13
    ...
    def reduce_armor(self, amount):
        ...
        # BEGIN Problem 13
        "*** YOUR CODE HERE ***"
        if amount >= self.armor and self.rank == 0:
            bees_win()
