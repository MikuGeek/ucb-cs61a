...
def roll_dice(num_rolls, dice=six_sided):
    ...
    for roll in range(num_rolls):
        result_in_roll = selected_dice()
        result_all_roll.append(result_in_roll)
    if 1 in result_all_roll:
        result = 1
    else:
        result = sum(result_all_roll)
    return result
    # END PROBLEM 1
...
def is_swap(player_score, opponent_score):
    ...
    judge_abs = abs(player_ones_digit - opponent_ones_digit)
    opponent_tens_digit = ((opponent_score - opponent_ones_digit) / 10) % 10
    if judge_abs == opponent_tens_digit:
        result = True
    else:
        result = False
    return result
...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    def take_turn_with_rules(strategy, score_p, score_o, score_add):
        break_jud = 0
        if feral_hogs == True:
            if abs(strategy(score_p, score_o) - score_add) == 2:
                score_ext = 3
            else:
                score_ext = 0
        else:
            score_ext = 0
        score_add = take_turn(strategy(score_p, score_o), score_o, dice)
        score_p += (score_add + score_ext)
        if is_swap(score_p, score_o):
            score_o, score_p = score_p, score_o
...
#            break
#
#        score1_add = take_turn(strategy1(score1,score0), score0, dice)
#        score1 += score1_add
