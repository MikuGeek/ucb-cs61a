...
def choose(paragraphs, select, k):
    ...
    # BEGIN PROBLEM 1
    "*** YOUR CODE HERE ***"
    selected_lst = [para for para in paragraphs if select(para)]
...
def accuracy(typed, reference):
    ...
    for i in range(min(len(reference_words), len(typed_words))):
        if typed_words[i] == reference_words[i]:
            accurate_length += 1
    return accurate_length / typed_length * 100.0
    # END PROBLEM 3
...
def wpm(typed, elapsed):
    ...
    # BEGIN PROBLEM 4
    "*** YOUR CODE HERE ***"
    costed_mintues = elapsed / 60.0
    return len(typed) / 5 /costed_mintues
    # END PROBLEM 4
